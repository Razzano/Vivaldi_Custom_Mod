...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  const CURRENT_MOD_ID   = '_srazzano_ui_mod_', // Current  Version 3
        OLD_PREFIX_V2    = 'srazzano_',         // Previous Version 2
        KNOWN_UNPREFIXED = [                    // Unprefix Version 1
  ...
  const $   = (sel, ctx = document) => ctx?.querySelector(sel) ?? null, // Usage: const main = $id('main'); $('.toolbar-statusbar', main);
        $$  = (sel, ctx = document) => ctx?.querySelectorAll(sel) ?? [], // Usage: const menu = $id('optionsMenu'); $$('.radio', menu).forEach(...);
        $id = (id) => document.getElementById(id), // Usage: $id('optionsButton');
		$c  = (type, props = {}, ...children) => {
          ...
          }); // Usage: $c('span', {id: 'calendar', className: 'aCal', title: Texts.dateTooltip, onmouseover: setDateText});
  ...
  const handleDrag = (event) => { event.preventDefault?.(); setTimeout(() => homeToRestart(s.homeRestart), s.resizeDelay)},
        handleFocus = () => setTimeout(() => reloadElements(), s.resizeDelay),
        handleLoad = () => setTimeout(() => initialize(), Timers.delayedInitialize),
        handleLayoutChange = () => { setTimeout(() => { reloadElements(); getToolbarList(s.toolbarList); }, s.resizeDelay)};

  const insertAfter = (newNode, ref) => {ref?.parentNode?.insertBefore(newNode, ref.nextSibling)}, // Usage: insertAfter(clk, cal);
        removeDupes = (className) => {
  ...
  const storage = {
    ...
    }, // Usage: await storage.getMod('hideTabbar', s.hideTabbar)
    ...
    }, // Usage: await storage.setMod('hideTabbar', s.hideTabbar);
    ...
    async clearAllMod() {
      try {
        const result = await chrome.storage.local.get(null),
              keysToRemove = Object.keys(result).filter(key => key.startsWith(CURRENT_MOD_ID));
  ...
  const Icons = {
	...
	mask16On:    'background: url(/style/icons/mask16On.png) center center no-repeat',
	...
	paintbrush:  'background: url(/style/icons/paintbrush.png) center center no-repeat',
	palette:     'background: url(/style/icons/palette.png) center center no-repeat',
	...
	selector:    'background: url(/style/icons/selector.png) center center no-repeat',
  ...
  const Texts = {
	...
	label0:             'Enter a non-conflicting keyCode/combination\n• Leave blank for no effect\n• Example: F9\n• Example: Ctrl+Alt+T',
    ...
    span2:              'Custom CSS For Vivaldi Issuna Theme',
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  async function initialize() {
	...
	const browser       = $id('browser'),
          main          = $id('main'),
          exts          = $('.toolbar-extensions > .button-toolbar', main),
          footer2       = $('.dialog-footer'),
		  tabsContainer = $id('tabs-container'),
          workspace     = $('.tabbar-workspace-button .button-title', tabsContainer);
    if (browser) {
  ...
  async function loadAllSettings() {
    ...
    try {
      ...
      const allData          = await chrome.storage.local.get(null),
            ...
            hasOldPrefixV2   = allKeys.some(key => key.startsWith(OLD_PREFIX_V2)),
            hasUnprefixed    = KNOWN_UNPREFIXED.some(key => allKeys.includes(key)),
            hasCurrentPrefix = allKeys.some(key => key.startsWith(CURRENT_MOD_ID)),
            needsMigration   = hasOldPrefixV2 || hasUnprefixed || !hasCurrentPrefix;
      if (needsMigration) {
        console.log(`[${CURRENT_MOD_ID}] Old data detected. Starting migration to version ${UPGRADE_VERSION}...`);
        await migrateToCurrentVer();           // Your migration function
	  ...
	  await storage.getMod('hideTabbar', s.hideTabbar);
      dateHolder();
      favImage(s.favInUrl);
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  async function migrateToCurrentVer() {
    const allData = await chrome.storage.local.get(null),
          allKeys = Object.keys(allData),
          newData = {};
    for (const key of KNOWN_UNPREFIXED) {
    ...
    if (Object.keys(newData).length > 0) {
      ...
      const keysToRemove = [
        ...KNOWN_UNPREFIXED.filter(k => allData[k] !== undefined),
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function dateHolder() {
	const main = $id('main'),
          statusBar = $('.toolbar-statusbar', main);
    if (!statusBar) return;
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function setOptionsMenu() {
	const browser = $id('browser'),
	      main    = $id('main'),
	      inner   = $('.inner', main),
          optMenu = $c('div', { id: 'optionsMenu', className: 'options-menu-popup', style: { display: 'none' }}),
          div0    = $c('div', { id: 'div0' });
    [1, 2, 3, 4].forEach(n => {
    ...
    optMenu.appendChild(Row2(2, null, Texts.span2, Texts.label2, Icons.palette, s.customCss));
    ...
    optMenu.appendChild(Row13(15, null, Texts.span15, Texts.label15, Icons.mask16On, s.toolbarToggle));
    ...
    try {
	  if (browser) {
        ...
        $id('input22').onclick = () => onOptionsMenuInput('input22');
        ...
        $id('span21').onclick = onClearField;
        $id('input0').oninput = () => onOptionsMenuInput('input0');
        $id('input10a').oninput = () => onOptionsMenuInput('input10a');
        $id('input10b').oninput = () => onOptionsMenuInput('input10b');
        $id('input0').value = s.keyCodes ?? 'F9';
        ...
        $id('span16a').style.opacity = document.getElementById('svg16').style.opacity = hideFooterOpacity;
        const hideBookmarkOpacity = s.hideBookmark ? '1' : '0.3';
        $id('span17a').style.opacity = document.getElementById('svg17').style.opacity = hideBookmarkOpacity;
        const hideMainbarOpacity = s.hideMainbar ? '1' : '0.3';
        $id('span18a').style.opacity = document.getElementById('svg18').style.opacity = hideMainbarOpacity;
        $id('button3').style.opacity = s.showDate ? '1' : '0.3';
        $id('button22').style.opacity = s.showTime ? '1' : '0.3';
    ...
    const config = {
      ...
      'input16': {
        save: () => {
          ...
          $id('span16a').style.opacity = $id('svg16').style.opacity = opacity;
      ...
      'input17': {
        save: () => {
          ...
          $id('span17a').style.opacity = $id('svg17').style.opacity = opacity;
      ...
      'input18': {
        save: () => {
          ...
          $id('span18a').style.opacity = $id('svg18').style.opacity = opacity;
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function getHomeButton() {
	const main = $id('main'),
          button = $('button[title^="Homepage"], button[title="Go to homepage"]', $('.toolbar-mainbar', main));
  ...
  function homeToRestart(bol) {
    ...
    if (bol) button.addEventListener('dragend', handleDrag);
    if (browser) {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function setToggleButton() {
    const tabs   = $id('tabs-container'),
		  toggle = $c('button', {id: 'toggle-toolbars', className: 'ToolbarButton-Button custom-button  toggle-toolbars', title: Texts.toggleTooltip, onclick: () => {setToolbars(); reloadElements(); if ($id('optionsMenu')) onOptionsMenuPosition(s.positionMenu)}});
	try {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  if (browser) {
	$id('key-code-input').addEventListener('change', async (e) => {
  ...
  async function initFullScreenMod() {
    ...
    const toggleFullScreen = async () => {
      ...
      if (isEnabled) applyListeners();
      else removeListeners();
	  const browser = $id('browser');
      if (browser) isEnabled ? browser.setAttribute('tabbar-hidden', 'true') : browser.removeAttribute('tabbar-hidden');
    };
    const applyListeners = () => {
      webView.addEventListener('pointerenter', tabbarHide);
      hoverDiv.addEventListener('pointerenter', tabbarShow);
      tabbarHide();
    };
    const removeListeners = () => {
      webView.removeEventListener('pointerenter', tabbarHide);
      hoverDiv.removeEventListener('pointerenter', tabbarShow);
      tabbarShow();
    };
	const handleKeyboardShortcut = (id, combination) => {
      if (combination === s.keyCodes) toggleFullScreen(); 
    };
    vivaldi.tabsPrivate.onKeyboardShortcut.addListener(handleKeyboardShortcut);
    if (isEnabled) {
      applyListeners();
      browser?.setAttribute('tabbar-hidden', 'true');
  } }
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function reloadElements() {
    const cal         = $id('calendar'),
	      optBtn      = $id('optionsButton'),
		  optMenu     = $id('optionsMenu'),
		  restartBtn  = $id('restart-browser'),
		  togToolbars = $id('toggle-toolbars'),
		  main        = $id('main'),
	      inner       = $('.inner', main);
	try {
	  if (!cal) dateHolder();
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function shutDown() {
	...
	Promise.all(KNOWN_UNPREFIXED.map(key => storage.setMod(key, settings[key])))
    ...
    window.removeEventListener('fullscreenchange', handleLayoutChange);
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  window.addEventListener('fullscreenchange', handleLayoutChange);
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  initInterval = setInterval(() => {
    if (!$id('optionsButton')) initialize();
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
      ...
      chrome.webNavigation.onCompleted.addListener((details) => {
        if (details.tabId < 0) {
          ...
          if (view) {
            ...
            view = $('.webpanel-stack > .visible webview');
  ...
  function showDialog(linkUrl, fromPanel) {
    ...
    webview.addEventListener('loadstart', function () {
      ...
      $id('progressBar-' + webviewId).style.display = 'block';
      if ($id('input-' + this.id) !== null) {
        $id('input-' + this.id).value = this.src;
      }
    });
    webview.addEventListener('loadstop', function () {
      $id('progressBar-' + webviewId).style.display = 'none';
    });
    ...
    fromPanel ? document.body.appendChild(divContainer) : $('.active.visible.webpageview').appendChild(divContainer);
  }
  ...
  function showWebviewOptions(webviewId, thisElement) {
    ...
    console.log($id(inputId) === null, webviewId);
    if (webview && $id(inputId) === null) {
  ...
  function getWebviewId() {
    let tempId = 0;
    while (true) {
      if ($id('dialog-' + tempId) === null) break;
  ...
  function setBackButtonContent(buttonBack) {
    const svg = $('.button-toolbar [name="Back"] svg');
    if (svg) {
  ...
  function setForwardButtonContent(forwardButton) {
    const svg = $('.button-toolbar [name="Forward"] svg');
    if (svg) {
