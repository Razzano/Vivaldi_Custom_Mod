(() => {
  ...
  const $c = (type, props = {}, evls = []) => { let node = document.createElement(type);
  ...
  const $q  = (el, bol = false) => bol ? document.querySelectorAll(el) : document.querySelector(el);
    ...
    chrome.tabs.query({currentWindow: true, active: true}, ([tab]) => {
      ...
      if (tab.favIconUrl) {
        ...
        if (tab.url.startsWith('vivaldi://')) {
          img.src = '/style/icons/vivaldi16.png';
        } else if (tab.url.startsWith('chrome-extension://')) {
          img.src = '/style/icons/extension.png';
        } else {
            img.src = '/style/icons/page.png';
      } }
      let current = $q('#currentI');
      if (current) current.src = img.src;
    });
  }

  function getCurrentTabUpdated() {
	const MIN_FAVICON_POLL_MS = 800,
          DEFAULT_FAVICON_POLL_MS = 1000;
    if (!favInterval || favInterval < MIN_FAVICON_POLL_MS) {
      favInterval = DEFAULT_FAVICON_POLL_MS;
    }
  ...
  function setOptionsMenu() {
	...
	try {
      optMenu.innerHTML = '\
        ...
        <label id="label2" class="label" disabled style="opacity: .4 !important;" title="' + label2Tooltip +'">\
