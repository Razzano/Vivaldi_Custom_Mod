...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  const MOD_ID = '_srazzano_ui_mod_'; // This Current Version 3
  const OLD_PREFIX = 'srazzano_';     // Previous Version 2
  const KNOWN_UNPREFIXED = [          // First Version 1
  ...
  // Usage: $('.toolbar-extensions > .button-toolbar'); or $('.toolbar-extensions > .button-toolbar', $('.toolbar-mainbar'));
  
  const $$ = (sel, ctx = document) => ctx?.querySelectorAll(sel) ?? [];
  // Usage: $$('#div0 > .radio'); or $$('#div0 > .radio', $id('optionsMenu'));

  const $id = (id) => document.getElementById(id);
  // Usage: $id('optionsButton'); or $id('optionsButton', $('.toolbar-statusbar'));

  const handleDrag = (event) => {
    event.preventDefault?.();
    setTimeout(() => homeToRestart(s.homeRestart), s.resizeDelay);
  };

  const handleFocus = () => setTimeout(() => reloadElements(), s.resizeDelay);
  ...
  const handleLoad = () => setTimeout(() => initialize(), Timers.delayedInitialize);

  const insertAfter = (newNode, ref) => {
	ref?.parentNode?.insertBefore(newNode, ref.nextSibling); // Usage: insertAfter(clk, cal);
  };
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  async function initialize() {
	const allData = await chrome.storage.local.get(null),
          ...
          hasOldPrefix = allKeys.some(key => key.startsWith(OLD_PREFIX)),
          hasUnprefixed = KNOWN_UNPREFIXED.some(key => allKeys.includes(key));
    if (hasOldPrefix || hasUnprefixed) {
      console.log(`[${MOD_ID}] Old data detected. Starting migration...`);
	  await migrateToCurrentV3();
    } else {
	  if (!hasMigrationData) {
        console.log(`[${MOD_ID}] Storage is up to date. No migration needed.`);
	...
	const browser   = $id('browser'),
          mainbar   = $('.mainbar'),
          exts      = $('.toolbar-extensions > .button-toolbar', mainbar),
          footer2   = $('.dialog-footer'),
          statusBar = $('.toolbar-statusbar'),
          workspace = $('.button-toolbar.tabbar-workspace-button > .ToolbarButton-Button > .button-title');
    if (browser) {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
// Migrate old prefixed / unprefixed keys from V1 to current MOD_ID when detected
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  async function migrateToCurrentV3() {
    try {
      const allData = await chrome.storage.local.get(null),
            keysToMigrate = [],
			keysToRemove = [],
      ...
      for (const key of Object.keys(allData)) {
        if (key.startsWith(OLD_PREFIX)) {
          ...
          const shortKey = key.slice(OLD_PREFIX.length),
                newFullKey = `${MOD_ID}${shortKey}`;
          if (key !== newFullKey) newData[newFullKey] = allData[key];
      } }
      for (const key of KNOWN_UNPREFIXED) {
        if (allData[key] !== undefined) keysToRemove.push(key);
      }
      if (Object.keys(newData).length > 0 || keysToRemove.length > 0) {
        if (Object.keys(newData).length > 0) await chrome.storage.local.set(newData);
        await chrome.storage.local.remove([...keysToMigrate, ...keysToRemove]);
        console.log(`[${MOD_ID}] Migration v1 → v2 complete. ` +
          `Moved ${keysToMigrate.length} keys and cleaned up ${keysToRemove.length} old unprefixed keys.`);
      }
    } catch (err) { console.error(`[${MOD_ID}] Migration from v1 failed:`, err); }
  }
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
// Date / calendar
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function dateHolder() {
    const statusBar = $('.toolbar-statusbar');
    if (!statusBar) return;
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function getHomeButton() {
    const button = $('button[title^="Homepage"], button[title="Go to homepage"]');
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  if (browser) {
	$id('key-code-input').addEventListener('change', async (e) => {
      ...
      console.log(`[${MOD_ID}] Shortcut updated to: ${newKey}`);
    });
  }
