...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  const UPGRADE_VERSION  = 3,
        CURRENT_MOD_ID   = '_srazzano_ui_mod_', // Current  Version 3
        OLD_PREFIX_V2    = 'srazzano_',         // Previous Version 2
        UNPREFIXED_V1 = [                       // Unprefix Version 1
  ...
  const handleDrag   = (event) => { event.preventDefault?.(); setTimeout(() => homeToRestart(s.homeRestart), s.resizeDelay)},
        ...
        handleLayout = () => { setTimeout(() => { reloadElements(); getToolbarList(s.toolbarList); }, s.resizeDelay)},
  ...
  async function loadAllSettings() {
    ...
    try {
      const allData          = await chrome.storage.local.get(null),
            ...
            hasUnprefixed    = UNPREFIXED_V1.some(key => allKeys.includes(key)),
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function timeHolder() {
    ...
    try {
      if (typeof s.timeFormat !== 'number' || s.timeFormat < 1 || s.timeFormat > 4) s.timeFormat = 1;
      clk.id = 'clock';
      insertAfter(div, cal);
