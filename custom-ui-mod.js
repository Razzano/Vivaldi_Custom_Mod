...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  const Texts = {
	...
	label0: '(1) Enter a non-conflicting keyCode/combination\n(2) Comma separate (space optional) multiple\n     entries for toggling the tabbar\n(3) Also hides toolbars ✓ in Toggle Toolbars Button\n(4) Leave input field blank for only Esc key toggling\n    • Example: F9\n    • Example: Alt+X\n    • Example: Ctrl+Alt+T\n    • Example: F9, Alt+X, Ctrl+Alt+T\nNOTE:\n Esc key is hard coded, and may need document\n focus after browser restarts to have any effect',
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  async function initTabbarMod() {
	...
	vivaldi.tabsPrivate.onKeyboardShortcut.addListener((id, combo) => {
      ...
      if (s.keyCodes && typeof s.keyCodes === 'string') {
        const keyList = s.keyCodes.split(',').map(k => k.trim()).filter(Boolean);
        if (keyList.includes(combo)) {
          toggleTabbar();
      } }
    });
    if (isEnabled) {
