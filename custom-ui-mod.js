...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  const INPUT_CONFIG = {
    openSearchSelected: ['Ctrl+Shift+S'],
  };
  const UI_CONFIG = {
    showUrlInput: false,
  };
  const ICON_CONFIG = {
    ...
    showIconDelay: 0,
    showPreviewOnHoverDelay: 0,
  };
  const IMAGE_CONFIG = {
	preview: '🔍 ', 
  };
  const KEYCODE_CONFIG = 'Ctrl+Alt+Period, Ctrl+Shift+F, Esc, Alt+4';
  const TIMING_CONFIG = {
  ...
  class PreviewWindow {
    ...
    constructor() {
	  ...
	  vivaldi.tabsPrivate.onKeyboardShortcut.addListener((id, combination) => {
		...
		const [first, second, third, fourth] = key_Code;
	    if (key_Position === first || key_Position === second) {
		  this.searchForSelectedText.bind(this);
        } else if (key_Position === third) {
          this.removePreview(webviewData.webview.id);
		} else if (key_Position === fourth) {
          this.removePreview(webviewData.webview.id);
		}
      });
  ...
  class PreviewRenderer {
    ...
    applyInitialSizing(previewWindow, stackIndex) {
	  previewWindow.style.width = 100 * stackIndex + '%';
      previewWindow.style.height = 100 * stackIndex + '%';
  ...
  class IconUtils {
    static SVG = {
	  ...
	  closeBtn:
		'<svg xmlns="http://www.w3.org/2000/svg" height="48px" width="48px" viewBox="0 0 48 48"><path d="M38 12.83L35.17 10 24 21.17 12.83 10 10 12.83 21.17 24 10 35.17 12.83 38 24 26.83 35.17 38 38 35.17 26.83 24z"/></svg>',
    };
