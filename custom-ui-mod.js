(() => {
  ...
  const $c = (type, props = {}, ...children) => {
    ...
    }); // Usage: statusSpan = $c('span', {id: 'status', className: 'd-flex'}, $c('span', { className: 'dot' }), 'Connected');
    return node;
  };

  const $q = (sel, all = false) => all ? document.querySelectorAll(sel) : document.querySelector(sel);
  // Usage: $q('#browser') or $q('.radio', true).forEach(radio => radio.checked = false);

  const insertAfter = (newNode, ref) => ref?.parentNode?.insertBefore(newNode, ref.nextSibling ?? null); // Usage: insertAfter(clk, cal);
  ...
  }; // single key only Usage: storage.get('dateFormatKey', 1); storage.set('dateFormatKey', dateFormat);

  const Icons = {
	bulb:        'background: url(/style/icons/bulb.png) center no-repeat',
	bookmark:    'background: url(/style/icons/bookmarkbar.png) center no-repeat',
	...
	footer:      'background: url(/style/icons/footer.png) center no-repeat',
	hourglass:   'background: url(/style/icons/hourglass.png) center no-repeat',
	main:        'background: url(/style/icons/mainbar.png) center no-repeat',
	...
	search:      'background: url(/style/icons/search.png) center no-repeat',
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function dateHolder() {
    const browser   = $q('#browser'),
	      statusBar = $q('.toolbar-statusbar'),
		  div       = $c('div', {id: 'calendar', className: 'aCal', title: Texts.dateTooltip, onclick: () => selectDateFormat()});
	try {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function timeHolder() {
    const cal = $q('#calendar'),
		  clk = $q('.ClockButton > button'),
	      div = $c('div', {id: 'digitalClock', className: 'aClk', title: Texts.customTime});
	try {
...
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
  function setOptionsMenu() {
	...
	function Row18(id, labelText, spanText, tooltip, iconStyle) {
	  ...
	  div.appendChild($c('input', {id: `input${id}a`, className: 'input input-timer', type: 'number', title: `${Texts.input10a}`, value: `${favInterval}`}));
	  div.appendChild($c('span', {id: `span${id}b`, className: '', textContent: `${Texts.resizeDelay}`, title: `${Texts.span10b}`}));
	  div.appendChild($c('input', {id: `input${id}b`, className: 'input input-timer', type: 'number', title: `${Texts.input10b}`, value: `${resizeDelay}`}));
	  iconSpan.appendChild($c('svg', {id: `svg${id}`, className: 'svg', xmlns: 'http://www.w3.org/2000/svg', style: iconStyle,}));
	...
	function Row19(id, labelText, spanText, tooltip) {
	  ...
	  label.appendChild($c('span', {id: `span${id}a`, textContent: '▼'}));
	  label.appendChild($c('span', {id: `span${id}b`, textContent: `${Texts.setOrder}`}));
	  label.appendChild($c('span', {id: `span${id}c`, textContent: '▼'}));
      return label;
    }
	function Row20(id, labelText, spanText, tooltip, iconStyle, value) {
	  ...
	  div.appendChild($c('input', {id: `input${id}a`, className: 'input', type: 'text', value: value}));
      iconSpan.appendChild($c('svg', {id: `svg${id}`, className: 'svg', xmlns: 'http://www.w3.org/2000/svg', style: iconStyle,}));
      div.appendChild(iconSpan);
	  return div;
	}
	...
	$q('#span16a').style.opacity = $q('#svg16').style.opacity = hideFooter ? '1' : '0.3';
	$q('#span17a').style.opacity = $q('#svg17').style.opacity = hideBookmark ? '1' : '0.3';
	$q('#span18a').style.opacity = $q('#svg18').style.opacity = hideMainbar ? '1' : '0.3';
